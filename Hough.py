...
def NegPosInversion(img_src):
    y = 255 - x
    y = np.clip(y, 0, 255).astype(np.uint8)
    img_dst = cv2.LUT(img_src, y)
    return img_dst

def Hough(img_gray, img_bgr):
    img_gray = NegPosInversion(img_gray)
    threshold = scl_threshold.get()
    mLL = scl_minLineLength.get()
    mLG = scl_maxLineGap.get()
    lines = cv2.HoughLinesP(img_gray, rho=1, theta=np.pi/360, threshold=threshold, minLineLength=mLL, maxLineGap=mLG)
    for line in lines:
        x1, y1, x2, y2 = line[0]
...
def HoughCircle(img_gray, img_bgr):
    img_gray = NegPosInversion(img_gray)
    circles = cv2.HoughCircles(img_gray,cv2.HOUGH_GRADIENT,1,20, param1=50,param2=30,minRadius=40,maxRadius=0)
...
def stop(event):
    ...
    img_hough = HoughCircle(img_gray, img_origin)
...
scl_threshold = tk.Scale(frame_parametor, orient="horizontal", from_=0, to=160, resolution=10)
scl_threshold.grid(row=0, column=1, sticky=tk.NSEW)
scl_threshold.set(80)

lbl_minLineLength = tk.Label(frame_parametor, text="minLineLength")
lbl_minLineLength.grid(row=1, column=0)

scl_minLineLength = tk.Scale(frame_parametor, orient="horizontal", from_=0, to=800, resolution=10)
